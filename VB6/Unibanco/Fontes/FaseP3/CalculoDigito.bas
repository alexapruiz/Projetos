...
Public Function Modulo11Fininvest(ByVal pviNumero As Long) As Byte

   '--------------------------------------------------------------------------
   '-------------         MODULO 11 (Envelope FININVEST)         -------------
   '--------------------------------------------------------------------------
   ' Calcular Dígito sobre 9 dígitos do Nr de Envelope.
   ' Pesos de 2 à 9 calculando da direita para a esquerda.
   ' Ex.:   T T N N N N N N N
   '        * * * * * * * * *
   '        2 9 8 7 6 5 4 3 2
   '        Obter o Resto da divisão onde, soma-se o total das multiplicações
   '        e divide-se por 11
   '        DV = 11 - Resto
   '        Se DV > 9 , então DV = DV - 10
   '--------------------------------------------------------------------------
   Dim soma As Integer
   Dim resto As Integer
   Dim digito_11 As Byte
   ...
   '---- se o calculo for Maior que 9 , muda-se. ----
   If digito_11 > 9 Then
      digito_11 = (digito_11 - 10)
   End If

   Modulo11Fininvest = digito_11
