...
CNTService::CNTService(const char* szServiceName)
{
    ...
    //Obtém o fullpath do aplicativo
	::GetModuleFileName(NULL, m_szFilePath, sizeof(m_szFilePath));
    ...
    m_bIsRunning = FALSE;
		...
		CFileException	E;
		CString			sPath;
		//Obtém o path do arquivo de log
		sPath =  m_szFilePath;
		sPath = sPath.Mid(0, strlen(m_szFilePath)-3);  
		sPath += "log";

		//Grava o header do arquivo	
		m_LogFile.Open(sPath,CFile::modeCreate | CFile::modeNoTruncate | CFile::modeWrite |CFile::typeText, &E);
		m_sLogMsg =  "------------------------------------------------------------\n";
		m_sLogMsg += "MDI_Ubb - VinculoAutomatico - Arquivo de Log\n";
		m_sLogMsg += "Data..."  + t.Format("%d/%m/%Y") + '\n';    
		m_sLogMsg += "Hora..."  + t.Format("%H:%M:%S") + '\n';    
		m_sLogMsg += "------------------------------------------------------------\n";
		m_LogFile.SeekToEnd();
		m_LogFile.WriteString(LPCSTR(m_sLogMsg));
	#endif
...
BOOL CNTService::ParseStandardArgs(int argc, char* argv[])
{
    ...
    if (_stricmp(argv[1], "-v") == 0) {
        ...
        printf("O servico %s instalado!\n",
               IsInstalled() ? "foi" : "nao foi");
        return TRUE; // say we processed the argument

	} else if (_stricmp(argv[1], "-J") == 0) {

		printf("%s - Author: Junior - Data: 15/01/2003", m_szServiceName);
		return true;
        ...
        // Request to install.
        if (IsInstalled()) {
            printf("%s ja foi instalado \n", m_szServiceName);
        } else {
            // Try and install the copy that's running
            if (Install()) {
                printf("O servico %s foi instalado!", m_szServiceName);
            } else {
                printf("O servico %s nao foi instalado. Erro: %d\n", m_szServiceName, GetLastError());
            }
        }
        return TRUE; // say we processed the argument
        ...
        if (!IsInstalled()) {
            printf("O servico %s nao foi instalado.", m_szServiceName);
        } else {
            ...
            if (Uninstall()) {
                ...
                printf("O servico %s foi removido. (O arquivo '%s' deve ser deletado manualmente.)",
                       m_szServiceName, m_szFilePath);
            } else {
                printf("O servico %s nao pode ser removido. Erro: %d", m_szServiceName, GetLastError());
            }
        }
        return TRUE; // say we processed the argument
...
{
    char aDependecies[32];

	strcpy(aDependecies, "MSSQLServer");
	aDependecies[12] = '\0';
    ...
    ::RegSetValueEx(hKey,
                    ...
                    0,
                    REG_EXPAND_SZ, 
                    (CONST BYTE*)m_szFilePath,
                    strlen(m_szFilePath) + 1);     
