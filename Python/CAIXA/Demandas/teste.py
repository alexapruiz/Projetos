...
def commit_dados(*args):
    ...
    c = connection.cursor()
    c.execute(
        'CREATE TABLE IF NOT EXISTS dados (_id integer , nome text, cpf integer, tel varchar(17), email varchar(150), data_nasc varchar(12), estado varchar(30), cidade varchar(30))')
    c.execute('INSERT INTO dados (_id ,nome, cpf, tel, email, data_nasc, estado, cidade) VALUES (?,?,?,?,?,?,?,?)',
              (args))
    connection.commit()
